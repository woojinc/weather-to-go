...
var renderMap = function renderMap(month) {
  var color = d3.scaleQuantize().domain([45, 90]).range(["#3288BD80", "#66C2A580", "#ABDDA480", "#E6F59880", "#FFFFBF80", "#FEE08B80", "#FDAE6180", "#F46D4380", "#D53E4F80", "#9E014280"]);
  ...
  var g = svg.append('g');
  var projection = d3.geoOrthographic().center(center); // .scale(200);
  ...
  enableRotation(); // drawGraticule();

  queue().defer(d3.json, "./data/world-110m2.json").defer(d3.json, "./data/tas-2016-".concat(month, ".json")).await(renderGlobalMap); // enableRotation();
  ...
  function renderGlobalMap(error, topology, temperature) {
    ...
    g.selectAll("path").data(geojson.features).enter().append("path").attr("d", path).style("fill", function (d) {
      return temperatureColor(d.id, temperature);
    }).style("stroke", "#eee").call(d3.drag().subject(function () {
      ...
      timer.stop();
      var rotate = projection.rotate(); // console.log(d3.event.x);

      projection.rotate([d3.event.x * sens, -d3.event.y * sens, rotate[2]]);
      svg.selectAll("path").attr("d", path); // svg.selectAll(".focused").classed("focused", focused = false);
    })).on("click", function (d) {
      clicked(d);
      Object(_selected_map__WEBPACK_IMPORTED_MODULE_1__["renderSelectedCountry"])("update", d, temperatureColor(d.id, temperature));
    }); // enableRotation();

    function clicked(d) {
      var x = 0,
          y = 0; // If the click was on the centered state or the background, re-center.
      ...
      g.transition().duration(750).attr("transform", "translate(" + x + "," + y + ")");
    }
  ...
  function enableRotation() {
    timer = d3.timer(function (elapsed) {
      projection.rotate([globeConfig.speed * elapsed - 120, globeConfig.verticalTilt, globeConfig.horizontalTilt]); // projection.rotate([90,0,0]);
  ...
  function drawGraticule() {
    ...
    g.selectAll("path").datum(graticule).enter().append("gpath").attr("class", "graticule").attr("d", path).style("fill", "transparent").style("stroke", "rgba(204, 204, 204, 0.60)");
  }
};
...
var renderSelectedCountry = function renderSelectedCountry(action, geojsonFeature, jsonCountryTemperature) {
  ...
  if (action === "update") {
    ...
    var scale = Math.min((bounds[1][0] - bounds[0][0]) / Math.PI, (bounds[1][1] - bounds[0][1]) / Math.PI);
